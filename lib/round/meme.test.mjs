...
test("rare slogan uses one consistent physical placement, never shirt plus whiteboard", async () => {
  try {
    // Non-classic styles have weight 38: .3 selects disposable, .1 allows text.
    const { prompts, textCalls } = setup([0.3, 0.1]);
    const result = await generateMeme(config, "plant waiting for sunlight", true);
    assert.equal(result.imageError, undefined);
...
test("image macro always carries its caption, exactly once, over the picture edge", async () => {
  try {
    // .45 of weight 38 lands on the caption style.
    const { prompts, textCalls } = setup([0.45]);
    const result = await generateMeme(config, "plant waiting for sunlight", true);
    assert.equal(result.imageError, undefined);
...
test("plain animal photo keeps it plain and names it like apeonfone", async () => {
  try {
    const { prompts, textCalls } = setup([]);
    const result = await generateMeme(config, undefined, true, "trend", { style: "plain-animal" });
    assert.equal(result.style, "plain-animal");
    assert.equal(result.slogan, "");
    assert.match(textCalls[0].instructions, /all lowercase, words mashed together/);
    assert.match(textCalls[0].instructions, /Never reuse an existing famous coin name/);
    assert.match(prompts[0], /plain, natural, sharp photo of the real animal/);
    assert.match(prompts[0], /No costume, no accessories/);
    assert.match(textCalls[0].input, /Comedy lens: plain relatable deadpan photo, no staging/);
    assert.doesNotMatch(textCalls[0].input, /bodyguard|knighted|crown|cape|superhero/);
    assert.match(textCalls[0].instructions, /Ignore any costume/);
    assert.match(textCalls[2].instructions, /any costume, vest, hat, glasses or other clothing on the animal is a blocking failure/);
  } finally { mock.restoreAll(); }
});
